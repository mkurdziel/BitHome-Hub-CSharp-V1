...
namespace SyNet.GuiHelpers.DragDropManager
{
  public static class DragDropManager
  {
    ...
    private static void DragSource_PreviewMouseLeftButtonDown( object sender, MouseButtonEventArgs e )
    {
      ...
      {
        // Check to see if we can find an item containe that is draggable
        Visual visual = e.OriginalSource as Visual;

        ItemsControl sourceItemsControl = sender as ItemsControl;

        if (sourceItemsControl == null) return;

        FrameworkElement sourceItemContainer = Utilities.GetItemContainer(sourceItemsControl, visual);

        if (sourceItemContainer != null)
        {
          if (CurrentDragSourceAdvisor.IsDraggable(sourceItemContainer as UIElement) == false)
          {
            return;
          }
          _draggedElt = sourceItemContainer as UIElement;
        }
      }
      else
      {
